    ...
    , bufferToUpdate(0)
{
    ...
    context.ddcci_data_handler = ddcci_data_handler;
...
void Screen::setEdid(std::vector<unsigned char>& edid) const
{
...
void Screen::ddcci_data_handler(evdi_ddcci_data ddcci_data, void* user_data)
{
    static_cast<Screen*>(user_data)->on_ddcci_change(ddcci_data);
}
...
void Screen::on_ddcci_change(evdi_ddcci_data ddcci_data)
{
    logging << "ddcci_data flags: " << ddcci_data.flags << ", buffer length: " << ddcci_data.buffer_length << ", buffer: " << *ddcci_data.buffer << std::endl;
}
